...
export const Todolist = (props: PropsType) => {
  const todolists: TodolistsType = useSelector<AppRootStateType, TodolistsType>(
    (state) =>
      state.todolists.find((tl) => tl.id === props.tlId) as TodolistsType,
  );
  ...
  return (
    <div className={s.todolist}>
      ...
      <ul className={s.list}>
        {tasks?.map((el) => {
          return <Task key={el.id} task={el} todoListId={tlId} />;
        })}
      </ul>
      <Grid
        item
